...
function requireWriteAllowed(config) {
    if (config.readOnly) {
        throw new Error("Serena plugin is configured in read-only mode");
    }
}
async function requireSession(manager, projectPath) {
    return manager.activateProject(projectPath);
}
export function registerSerenaTools(api, manager, config) {
    api.registerTool({
...
async function callNormalized(session, config, toolName, args) {
...
function isDenied(config, toolName) {
...
function looksLikeWriteTool(toolName) {
